  ...
  return (
    <div class="flex flex-col bg-dark-5 w-86 p-2 h-full max-h-xl mb-4">
      ...
      <div class="flex mt-2 w-full">
        <button
          class="appearance-none border-none outline-none p-2 color-white cursor-pointer w-full rounded-sm"
          classList={{
            ['bg-blue-7 hover:bg-blue-6']: activeView() === 0,
            ['bg-transparent']: activeView() !== 0,
          }}
        ...
        <button
          class="appearance-none border-none outline-none p-2 color-white cursor-pointer w-full rounded-sm"
          classList={{
            ['bg-blue-7 hover:bg-blue-6']: activeView() === 1,
            ['bg-transparent']: activeView() !== 1,
          }}
      ...
      <div class="custom-v-scrollbar flex flex-col mt-4 overflow-auto pr-2 select-none h-full">
        <Show when={activeView() === 1}>
          <p class="text-sm color-dark-2"> {'<div id="app">'} </p>
        </Show>
        ...
        <Show when={activeView() === 1}>
          <p class="text-sm color-dark-2"> {'</div>'} </p>
        </Show>
      </div>
    </div>
  );
...
const TreeView = (props: TreeViewProps) => {
  ...
  const depthMargin = 10 * props.depth;
    ...
    >
      ...
      >
        <p class="hover:bg-dark-4" onClick={() => props.selectLayer(props.id)}>
          ...
          <span class="color-indigo-3">
            id=<span class="color-green-3">"{props.id}"</span>
          </span>{' '}
          <span class="color-indigo-3">
            class=<span class="color-green-3">"{props.name}"</span>
          </span>
